// Battleships using state channels
// Things to change:
// - shoot
...
// Global Constants
var CONST = {};
CONST.AVAILABLE_SHIPS = ['Mini Boat #1', 'Mini Boat #2', 'Mini Boat #3', 'Mini Boat #4',
			 'Patrol Boat #1', 'Patrol Boat #2', 'Patrol Boat #3',
			 'Destroyer #1', 'Destroyer #2', 'Battleship'];
...
Game.usedShips = [CONST.UNUSED, CONST.UNUSED, CONST.UNUSED, CONST.UNUSED, CONST.UNUSED,
		  CONST.UNUSED, CONST.UNUSED, CONST.UNUSED, CONST.UNUSED, CONST.UNUSED];
CONST.USED = 1;
CONST.UNUSED = 0;
...
// player 1 hit -> (wait for timeout) -> player 2 reveal -> player 1    
...
Game.prototype.placementMouseover = function(e) {
	...
	if (self.placingOnGrid) {
	    ...
	    // console.log(x, y, fleetRoster, Game.placeShipType);

		for (var i = 0; i < fleetRoster.length; i++) {
		    var shipType = fleetRoster[i].type;
		    // console.log(shipType, Game.placeShipType);
...
Game.prototype.placeRandomly = function(e){
    ...
    commit(e.target.self.humanFleet);
...
Fleet.prototype.populate = function() {
	for (var i = 0; i < this.numShips; i++) {
	    ...
	    this.fleetRoster.push(new Ship(CONST.AVAILABLE_SHIPS[i], this.playerGrid, 
						 this.player));
	}
};
...
function Ship(type, playerGrid, player) {
    ...
    if (this.type.startsWith('Mini Boat')) {
	this.shipLength = 1;
    }
    else if (this.type.startsWith('Patrol Boat')) {
	this.shipLength = 2;
    }
    else if (this.type.startsWith('Destroyer')) {
	this.shipLength = 3;
    }
    else {
	this.shipLength = 4;
